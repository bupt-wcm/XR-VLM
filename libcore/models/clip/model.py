    ...
    def forward(self, x):
        ...
        pos_emb = torch.cat(
            [
                mean_pos_emb,
                func.interpolate(
                    patch_pos_emb.reshape(1, self.spacial_dim, self.spacial_dim, embed_dim).permute(0, 3, 1, 2),
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        return checkpoint_sequential(self.resblocks, segments=2, input=x, use_reentrant=False)
...
def build_model(state_dict: dict):
    ...
    if vit:
        ...
        if 'visual.stride_size' in state_dict and state_dict['visual.stride_size'] is not None:
            vision_stride_size = state_dict["visual.stride_size"]
        else:
            vision_stride_size = vision_patch_size
